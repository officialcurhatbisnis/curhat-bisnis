...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ============================================
    // CURHAT FORM HANDLING - FORMSPREE
    // ============================================
    // Formspree akan menangani submit secara otomatis
    // Kita hanya menampilkan loading state saja
    ...
    if (curhatForm) {
        curhatForm.addEventListener('submit', function(e) {
            ...
            // Formspree akan meng-handle submit dan redirect
            // Tidak perlu preventDefault karena kita pakai action Formspree
            
            // Generate ticket number untuk ditampilkan setelah redirect
            const randomTicket = 'CB-' + new Date().getFullYear() + '-' + 
                Math.floor(100000 + Math.random() * 900000);
            
            // Simpan ticket di localStorage untuk ditampilkan setelah redirect
            localStorage.setItem('curhatTicket', randomTicket);
        });
    }
    
    // Cek jika user baru saja submit form (dari redirect Formspree)
    function checkFormSubmitSuccess() {
        const urlParams = new URLSearchParams(window.location.search);
        const terkirim = urlParams.get('terkirim');
        const ticket = localStorage.getItem('curhatTicket');
        
        if (terkirim === 'true' && ticket) {
            // Tampilkan success message
            curhatOptions.style.display = 'none';
            curhatForm.style.display = 'none';
            successMessageEmail.classList.add('active');
            document.getElementById('ticketNumberEmail').textContent = ticket;
            
            // Clear ticket dari localStorage
            localStorage.removeItem('curhatTicket');
            
            // Hapus parameter dari URL
            window.history.replaceState({}, document.title, window.location.pathname);
            
            // Scroll ke bagian curhat
